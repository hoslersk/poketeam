...
class App extends React.Component {
  ...
  addPokemon(event) {
    ...
    const pokedexNumber = event.target.src.split(/\/|\./)[10]
    const copyOfState = Object.assign({},this.state)
    ...
    var myInit = { method: 'GET',
                   ...
                   mode: 'cors',
                   cache: 'default' };
    // fetchPokemonData(pokedexNumber);
    const pokemon = fetch(`https://pokeapi.co/api/v2/pokemon/${pokedexNumber}`, myInit).then(response => {
  ...
  fetchResults(array) {
    ...
    if (this.state.searchQuery == "") {
      ...
      return array.map((nameString, index) => {
        counter++
        if ((index % 5 === 0) && (counter < 50)) {
          return(
            <tr>
              <td className="poke-results">
                <span onClick={this.addPokemon}>
    ...
    return (
      <div className="App">
        ...
        <div className="poke-table">
          <h3>Simply click on up to 6 Pokémon sprites to complete team!</h3>
          <table>
            <tbody>
              {resultElements}
            </tbody>
